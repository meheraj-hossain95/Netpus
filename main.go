...
var (
	installFlag   = flag.Bool("install", false, "Install Octopus (create shortcuts)")
	uninstallFlag = flag.Bool("uninstall", false, "Uninstall Octopus (remove shortcuts)")
...
func main() {
	...
	// Handle CLI flags
	if *versionFlag {
		fmt.Printf("Octopus Network Monitor v%s\n", version)
	...
	if *installFlag {
		...
		fmt.Println("Octopus installed successfully!")
		os.Exit(0)
	}
	...
	if *uninstallFlag {
		...
		fmt.Println("Octopus uninstalled successfully!")
		os.Exit(0)
	}
